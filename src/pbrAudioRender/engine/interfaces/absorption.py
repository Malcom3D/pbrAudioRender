...
#    @nb.jit(nopython=True, parallel=True)
    def _apply_absorption(self, pressure: np.ndarray, vx: np.ndarray, vy: np.ndarray, vz: np.ndarray, boundaries: Dict) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        for boundary_idx in nb.prange(len(boundaries['impedance_discontinuities'])):
            ...
            absorption_factor = soxel_grid.soxels[i, j, k].acoustic_shader.acoustic_properties.absorption.get_avg_coeffs(self.low_freq, self.high_freq)
