...
class RayData:
    ...
    origins_idx: np.ndarray = field(default_factory=lambda: np.zeros((0, 1), dtype=np.int32))
    origins_bands: np.ndarray = field(default_factory=lambda: np.zeros((0, 1), dtype=np.int32))
...
class AcousticRayTracer:
    ...
    def __init__(self, config_file: str):
        ...
        self.acoustic_properties: Optional[AcousticProperties] = None
    ...
    def _initialize_scene(self):
        ...
        alpha, beta = self._compute_acoustic_domain_coefficients(
            sound_speed, density, temperature, impedance
        )
        ...
        self.acoustic_properties = AcousticProperties(
    ...
    def _initialize_sources(self):
        ...
        for src_config in self.config.sources:
            ...
            self.ray_data.origins_bands = np.append(self.ray_data.origins_bands, source_bands, axis=0)
            
            source_idx = np.full((self.n_points, 1), [src_config.idx], dtype=np.int32)
            self.ray_data.origins_idx = np.append(self.ray_data.origins_idx, source_idx, axis=0)
            
            source_arr = np.full((self.n_points, 3), [source_pos], dtype=np.float32)
            self.ray_data.origins = np.append(
                self.ray_data.origins, source_arr, axis=0
            )
    ...
    def _initialize_outputs(self):
        ...
        for out_config in self.config.outputs:
            ...
            self.ray_data.destinations_idx = np.append(self.ray_data.destinations_idx, output_idx, axis=0)
            ...
            self.geometry.mesh_info = np.append(
                self.geometry.mesh_info, mesh.vertices[mesh.faces], axis=0
            )
            ...
            n_faces = vertices[faces].shape[0]
            self.acoustic_properties.roughness = np.append(self.acoustic_properties.roughness, np.full((n_faces, 1), 0.0, dtype=np.float32), axis=0)
            
            for prop_name in ['absorption', 'reflection', 'refraction', 'scattering']:
                coeffs = getattr(self.acoustic_properties, f'{prop_name}_coeffs')
                phases = getattr(self.acoustic_properties, f'{prop_name}_phases')
                ...
                setattr(self.acoustic_properties, f'{prop_name}_coeffs', np.append(coeffs, new_coeffs, axis=0))
                setattr(self.acoustic_properties, f'{prop_name}_phases', np.append(phases, new_phases, axis=0))
    ...
    def _initialize_objects(self):
        ...
        for obj_config in self.config.objects:
            ...
            self.geometry.mesh_info = np.append(
                self.geometry.mesh_info, vertices[faces], axis=0
            )
            self.geometry.scene_info = np.append(
                self.geometry.scene_info,
                np.full((vertices[faces].shape[0],), [obj_config.idx], dtype=np.int32)
            )
            
            # Get object acoustic properties
            n_faces = vertices[faces].shape[0]
            roughness = obj_config.acoustic_shader.roughness
            self.acoustic_properties.roughness = np.append(
                self.acoustic_properties.roughness,
                np.full((n_faces, 1), [roughness], dtype=np.float32),
                axis=0
            )
            ...
            for prop_name in ['absorption', 'reflection', 'refraction', 'scattering']:
                ...
                existing_coeffs = getattr(self.acoustic_properties, f'{prop_name}_coeffs')
                existing_phases = getattr(self.acoustic_properties, f'{prop_name}_phases')
                
                setattr(self.acoustic_properties, f'{prop_name}_coeffs', np.append(existing_coeffs, np.full((n_faces, self.n_bands), coeffs, dtype=np.float32), axis=0))
                setattr(self.acoustic_properties, f'{prop_name}_phases', np.append(existing_phases, np.full((n_faces, self.n_bands), phases, dtype=np.float32), axis=0))
        
        # Create Embree scene
        embree_mesh = TriangleMesh(self.scene, self.geometry.mesh_info)
    
    def _compute_acoustic_domain_coefficients(self, c: float, rho: float, 
                                               T: float, Z: float):
    ...
    def _compute_object_coefficients(self, c: float, rho: float, E: float, 
                                      nu: float, damping: float):
    ...
    def _initialize_directions(self):
        ...
        self.ray_data.energies = np.full((num_dirs, 1), 1.0, dtype=np.float32)
        self.ray_data.phases = np.full((num_dirs, 1), 0.0, dtype=np.float32)
        self.ray_data.delay = np.full((num_dirs, 1), 0.0, dtype=np.float32)
    ...
    def _update_medium_properties(self, path_length: np.ndarray):
        ...
        medium_speed = np.full((n_rays, 1), 
                               self.medium_properties.speed, 
                               dtype=np.float32)
        medium_alpha = np.full((n_rays, self.n_bands), 
                               self.medium_properties.alpha, 
                               dtype=np.float32)
        medium_beta = np.full((n_rays, self.n_bands), 
                              self.medium_properties.beta, 
                              dtype=np.float32)
        ...
        print('######DEBUG: ', origins_bands_idx.dtype, self.ray_data.origins_bands.dtype)
    ...
    def _collect_output_data(self, hits_obj_idx: np.ndarray, 
    ...
    def _continue_tracing(self, inters: np.ndarray, primID: np.ndarray,
                           path_length: np.ndarray, intersect_mask: np.ndarray):
    ...
    def _apply_material_properties(self, primID: np.ndarray, inters: np.ndarray, intersect_mask: np.ndarray, normals: np.ndarray):
        ...
        abs_coeffs = self.acoustic_properties.absorption_coeffs[primID_filtered][origins_bands_idx, bands]
        abs_phases = self.acoustic_properties.absorption_phases[primID_filtered][origins_bands_idx, bands]
        refl_coeffs = self.acoustic_properties.reflection_coeffs[primID_filtered][origins_bands_idx, bands]
        refl_phases = self.acoustic_properties.reflection_phases[primID_filtered][origins_bands_idx, bands]
        scat_coeffs = self.acoustic_properties.scattering_coeffs[primID_filtered][origins_bands_idx, bands]
        scat_phases = self.acoustic_properties.scattering_phases[primID_filtered][origins_bands_idx, bands]
        ...
        self.ray_data.origins_idx = np.append(self.ray_data.origins_idx, scattered_data['origins_idx'], axis=0)
        self.ray_data.origins_bands = np.append(self.ray_data.origins_bands, scattered_data['origins_bands'], axis=0)
        self.ray_data.destinations = np.append(self.ray_data.destinations, scattered_data['destinations'], axis=0)
        self.ray_data.destinations_idx = np.append(self.ray_data.destinations_idx, scattered_data['destinations_idx'], axis=0)
    ...
    def _compute_reflection_directions(self, incident_directions: np.ndarray,
                                        normals: np.ndarray,
                                        incident_angles: np.ndarray) -> np.ndarray:
    ...
    def _generate_scattering_rays(self, origins: np.ndarray, normals: np.ndarray, scat_coeffs: np.ndarray) -> Dict[str, np.ndarray]:
        ...
        roughness = self.acoustic_properties.roughness
        ...
        for src_idx in range(n_src):
            for out_idx in range(n_outs):
                for bands_idx in range(n_bands):
                    ...
                    self._compute_spherical_harmonics(
                        ambisonics_ir, filtered_delay, complex_amplitudes,
                        theta, phi, ambisonic_order
                    )
    ...
    def _compute_spherical_harmonics(self, ambisonics_ir: np.ndarray,
    ...
    def _convolve_with_audio(self):
        ...
        for out_config in self.config.outputs:
            for src_config in self.config.sources:
                ...
                multi_bands_audio = _mono_to_bands(
                    file_audio, sample_rate, self.frequency_bands.get_bands()
                )
                ...
                for bands_idx in range(multi_bands_audio.shape[0]):
                    ...
                    ambisonics_output = self._convolve_mono_to_ambisonics(
                        mono_audio, ambisonics_ir.T
                    )
                    
                    # Normalize
                    ambisonics_output_normalized = self._normalize_ambisonics_output(
                        ambisonics_output
                    )
    ...
    def _convolve_mono_to_ambisonics(self, mono_audio: np.ndarray,
                                      ambisonics_ir: np.ndarray,
    ...
    def _normalize_ambisonics_output(self, ambisonics_output: np.ndarray,
                                      normalize_individually: bool = False) -> np.ndarray:
...
def main():
    ...
    tracer = AcousticRayTracer(args.config)
    tracer.run()
