...
class GPUConfig:
    ...
    use_gpu: bool = True
    compute_device: str = "cuda"  # cuda, hip, oneapi, opencl
    device_id: int = 0
    num_streams: int = 4
...
class GPUManager:
    ...
    def _initialize_gpu(self):
        ...
        try:
            if self.config.compute_device == "cuda":
                self._initialize_cuda()
            elif self.config.compute_device == "hip":
                self._initialize_hip()
            elif self.config.compute_device == "oneapi":
                self._initialize_oneapi()
            elif self.config.compute_device == "opencl":
                self._initialize_opencl()
            else:
                warnings.warn(f"Unsupported GPU backend: {self.config.compute_device}")
    ...
    def _initialize_cuda(self):
        ...
        try:
            ...
            # Set device
            self.Device(self.config.device_id).use()
            self.device = self.Device(self.config.device_id)
            
            # Create streams
            for i in range(self.config.num_streams):
                self.streams.append(Stream(non_blocking=True))
                
            print(f"CUDA initialized on device {self.config.device_id}: {self.device.name}")
    ...
    def _initialize_hip(self):
        ...
        try:
            ...
            print(f"HIP initialized on device {self.config.device_id}: {self.device.name}")
    ...
    def _initialize_oneapi(self):
        ...
        try:
            ...
            if devices and self.config.device_id < len(devices):
                self.device = devices[self.config.device_id]
            else:
    ...
    def _initialize_opencl(self):
        ...
        try:
            ...
            if self.config.device_id < len(devices):
                self.device = devices[self.config.device_id]
            else:
            ...
            self.streams = [cl.CommandQueue(self.context) for _ in range(self.config.num_streams)]
    ...
    def allocate_memory(self, shape: Tuple[int, ...], dtype: np.dtype) -> Any:
        """Allocate GPU memory"""
        ...
        # Check memory limits
        if (self.config.memory_limit and 
            self.memory_allocated + nbytes > self.config.memory_limit * 1024 * 1024):
    ...
    def to_gpu(self, numpy_array: np.ndarray, stream_id: int = 0) -> Any:
        ...
        if not self.config.use_gpu or numpy_array is None:
            return numpy_array
        ...
        try:
            ...
            warnings.warn(f"GPU transfer failed: {e}. Using CPU array.")
            return numpy_array
    
    def to_cpu(self, gpu_array: Any, stream_id: int = 0) -> np.ndarray:
        """Transfer GPU array back to CPU"""
        if not self.config.use_gpu or gpu_array is None:
            return gpu_array
        ...
        try:
            ...
            warnings.warn(f"GPU to CPU transfer failed: {e}")
